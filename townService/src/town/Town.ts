...
import { CurrencyResponse } from '../api/Model';
...
export default class Town {
  ...
  /**
   * Updates currency for a player in this town based on the give userId and currency amount
   * @param userId the id of the player to update
   * @param currency the amount of currency to add to the player's current currency
   */
  public updateCurrency(currencyResponse: CurrencyResponse): void {
    this._broadcastEmitter.emit('currencyUpdate', currencyResponse);
  }

  /**
   * Updates the inventory of a player in this town based on the given userId and inventory
   * @param userId the id of the player to update
   * @param inventory the new inventory to set for the player
   */
  public updateInventory(inventoryResponse: {items: ItemDetails[], userID: string}): void {
    inventoryResponse.items.sort((a, b) => (a.slot > b.slot ? 1 : -1));
    this._broadcastEmitter.emit('inventoryChanged', inventoryResponse);
  }

  public updateTrades(trades: Trades[]) {
    this._broadcastEmitter.emit('tradeChanged', trades);
  }

  public createTrade(tradeCreated: Trades) {
    this._broadcastEmitter.emit('tradeCreated', tradeCreated);
  }

  public acceptTrade(tradeCreated: Trades) {
    this._broadcastEmitter.emit('tradeAccepted', tradeCreated);
  }  
