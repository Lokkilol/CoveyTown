...
import { TownsService, TownsServiceClient, TradeResponse, Trades } from '../generated/client';
...
import { CurrencyResponse } from '../generated/client/models/CurrencyResponse';
...
export type TownEvents = {
  ...
  /**
   * An event that indicates that the player's currency has changed
   * @param newCurrency the new currency value
   */
  currencyChanged: (newCurrency: CurrencyResponse) => void;

  /**
   * An event that indicates that the player's inventory has changed
   * @param inventoryResponse the new inventory value and the user id
  */
  inventoryChanged: (inventoryResponse : {items: ItemDetails[], userID: string}) => void;

  /**
   * An event that indicates that the global trades has
   * @param inventoryResponse the new inventory value and the user id
  */
  tradeChanged: (trades: Trades[]) => void;

  tradeCreated: (tradeResponse: {trades: Trades[], userId: string}) => void;
...
export default class TownController extends (EventEmitter as new () => TypedEmitter<TownEvents>) {
  ...
  /**
   * Distrubutes currency to the given userId by calling towsService API for the backend
   */
  public async distributeCurrency(
    ...
    requestBody.townID = this.townID;
    await this._townsService.distributeCurrency(userId, requestBody);
  }
  ...
  public async addItemToInventory(
      ...
      await this._townsService.addToInventory(this.sessionToken, { slot: slot, item_name: item_name, item_id: item_id, user_id: user_id })
    }

  /**
   * Gets the currency of the given userId by calling towsService API for the backend
   */
  public async getCurrency(userId: string): Promise<CurrencyResponse> {
    const currencyResponse = await this._townsService.getCurrency(userId, this.townID,);
    return currencyResponse;
  }

  public async getUserName(userId: string) : Promise<string> {
    const userNameResponse = await this._townsService.getUserName(userId);
    return userNameResponse;
  }

  /**
   * Gets the player's current login streak status by calling towsService API for the backend
   */
  public async getLoginStreakStatus(userId: string): Promise<{
    loggedInToday: boolean;
    currencyAwarded: number;
    streak: number;
}> {
    const loginStreakStatusResponse = await this._townsService.getLoginStreak(userId, this.townID); 
    return loginStreakStatusResponse;
  }

  /**
   * Creates a trade that gets added to the global trades list. 
   * @param slot slot of the item for the inventory of the player.
   * @param trade the item trade information
   */
  public async createTrade(slot: number, trade: CreateTrade): Promise<void> {
    await this._townsService.createTrade({slot, trade});
  }

  public async getActiveTrades(): Promise<Trades[]> {
    const activeTrades = await this._townsService.getActiveTrades();
    return activeTrades.trades;
  }

  public async acceptTrade(tradeId: number, buyerId: string): Promise<TradeResponse> {
    const result = await this._townsService.acceptTrade(tradeId, buyerId);
    return result;
  }
  ...
  registerSocketListeners() {
    ...
    /**
     * When a player's currency changes, update the local state and currency of the player
     */
    this._socket.on('currencyUpdate', (currencyResponse : CurrencyResponse) => {
      this.emit('currencyChanged', currencyResponse)
    });
    this._socket.on('inventoryChanged', (inventoryResponse : {items: ItemDetails[], userID: string}) => {
      this.emit('inventoryChanged', inventoryResponse)
    });

    this._socket.on('tradeChanged', (trades: Trades[]) => {
      this.emit('tradeChanged', trades)
    });

    this._socket.on('tradeCreated', (trade : { trades: Trades[], userId: string }) => {
      this.emit('tradeCreated', trade)
    });
    
    this._socket.on('tradeAccepted', (trade: Trades[]) => {
  ...
  async getShopItems(): Promise<ItemShopDetails[]> {
    const response = (await this._townsService.getShopItems())
    const shopItems = response.items;
    const items: ItemShopDetails[] = [];
  ...
  async getItemById(item_id: number): Promise<ItemWithCost> {
    const response = (await this._townsService.getItemById(item_id));
    console.log(response);
    const itemsWithCost = response.data;
  ...
  // Gets the inventory of the userId by calling the API for the backend, and sorting by slot order.
  public async getPlayerInventory(userId: string): Promise<ItemDetails[]> {
    const response = (await this._townsService.getInventoryByUserId(userId, this.townID))
    const inventories = response.items;
    const items: ItemDetails[] = [];
    for (const item of inventories) {
      items.push(item);
    }
    items.sort((a, b) => (a.slot > b.slot ? 1 : -1));
    return items;
  }
