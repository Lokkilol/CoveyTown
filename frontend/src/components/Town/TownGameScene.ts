...
export default class TownGameScene extends Phaser.Scene {
  ...
  constructor(coveyTownController: TownController, resourcePathPrefix = '') {
    super('TownGameScene');
    console.log(resourcePathPrefix);
  ...
  preload() {
    ...
    this.load.image('CoveyCoin', this._resourcePathPrefix + '/assets/CoveyCoin.png');
  ...
  create() {
    ...
    const label = this.add
      .text(spawnPoint.x, spawnPoint.y - 20, this.coveyTownController.ourPlayer.userName, {
        font: '18px monospace',
        color: '#03a1fc',
